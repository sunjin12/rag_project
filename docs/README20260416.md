...
| **AI/ML** | Ollama + qwen3:8b (LLM), BGE-M3 (임베딩), sentence-transformers |
...
```bash
# LLM 모델 다운로드
docker exec rag_ollama ollama pull qwen3:8b
```
...
| `LLM_MODEL` | Ollama LLM 모델명 | `qwen3:8b` |
...
RAG 파이프라인의 검색 품질(유사도)을 자동으로 기록하고, matplotlib 기반 리포트로 시각화할 수 있습니다.

### 동작 방식
- Qdrant 검색 결과의 유사도(score)를 매 채팅마다 DB(`rag_evaluations`)에 자동 기록
- 평가 항목: 평균/최소/최대 유사도, 반환 청크 수, 질문, 페르소나, 메시지 연결
- 평가 로깅은 ask/stream 엔드포인트 모두 지원, 실패해도 채팅 응답에 영향 없음
...
```
- `reports/` 디렉터리에 PNG 차트 3종 자동 생성

| 차트 | 파일 | 설명 |
|------|------|------|
| 유사도 추이 | similarity_trend.png | 일별 평균 유사도 + 질의 수 |
| 페르소나별 분포 | persona_distribution.png | 박스플롯으로 품질 비교 |
| 저품질 비율 | low_quality_trend.png | 유사도 < 0.3 비율 추이 |

### 검증 방법
1. 채팅 3회 → 평가 데이터 3건 기록 확인
2. `python scripts/eval_report.py` 실행 → PNG 차트 3종 생성 확인
3. README에 평가 시스템 설명/사용법/차트 예시가 명확히 반영
...
- 단계별 로딩 애니메이션 ("문서 검색 중..." → "응답 생성 중..." + 점 애니메이션)
...
### 페르소나 관리
- 페르소나 생성 및 상세 조회
- 스와이프하여 삭제 (Dismissible) — 대화 기록, 파일, 벡터 모두 캐스케이드 삭제
- 삭제 전 확인 다이얼로그
...
- [x] 한국어 LLM 지원 (qwen3:8b) + UTF-8 스트리밍
...
- [x] 메시지 텍스트 선택/복사
- [x] 다중 파일 업로드 스크롤 UI
- [x] 페르소나 스와이프 삭제 (캐스케이드: DB + Qdrant + Redis)
- [x] AI 응답 단계별 로딩 애니메이션 (SSE [STATUS] 이벤트)
- [x] RAG 검색 품질 평가 시스템 (유사도 로깅 + 리포트)
- [x] LLM 모델 외부 설정 (pydantic-settings, .env)
